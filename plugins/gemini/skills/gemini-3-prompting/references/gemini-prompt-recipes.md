...
Use these as starting templates for Gemini task prompts or other Gemini 3 prompt construction.
...
For non-code delegation (writing, brainstorming, explanation, analysis) prefer the later recipes.
Skip `<grounding_rules>` and `<citation_rules>` for creative or generative tasks where those blocks would overconstrain the output.
...
Use for creative or generative writing tasks: drafts, emails, documents, copy, summaries.

```xml
<task>
Draft the requested content in the requested voice and length.
Match the user's tone, audience, and format constraints.
</task>

<compact_output_contract>
Return only the requested content.
Do not add meta-commentary, reasoning trails, or options lists unless asked.
</compact_output_contract>

<default_follow_through_policy>
Produce a complete first draft. If the spec is ambiguous, pick the most reasonable interpretation and note it in one line at the end.
</default_follow_through_policy>
```

## Brainstorming / Ideation

Use when the user wants a range of options, not one answer.

```xml
<task>
Generate N distinct options for the stated problem.
Each option should be meaningfully different in approach, not a rephrasing.
</task>

<structured_output_contract>
Return an ordered list. For each option:
1. short label
...
Avoid filler entries that repeat earlier ideas.
</dig_deeper_nudge>
```

## Explanation / Teaching

Use when the user wants to understand something. State the audience level explicitly.

```xml
<task>
Explain the requested topic clearly to the stated audience.
Build from what the audience already knows toward the new concept.
</task>

<completeness_contract>
Cover the concept end-to-end. Do not stop at a definition when the user needs the mental model.
Use a concrete example to ground abstract points.
</completeness_contract>

<compact_output_contract>
Prefer plain prose and short examples over bullet-point sprawl.
Flag any important simplifications you made.
</compact_output_contract>
```

## Analysis / Comparison

Use for comparing options, evaluating tradeoffs, or decomposing a problem.

```xml
<task>
Analyze the stated options against the stated criteria.
Identify the decisive differences, not a uniform feature matrix.
</task>

<structured_output_contract>
Return:
1. summary of the decision
2. criteria that actually discriminate
3. matrix of options × decisive criteria
4. recommendation and its main risk
</structured_output_contract>

<verification_loop>
Before finalizing, check that the recommendation is consistent with the decisive criteria and that the main risk is stated honestly.
</verification_loop>
```
...
```xml
<task>
Diagnose why this existing prompt is underperforming and propose the smallest high-leverage changes to improve it for Gemini 3.
</task>
