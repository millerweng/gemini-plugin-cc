...
**重启不是可选项。** 会话只在启动时解析一次插件路径，之后一直用那一份。
升级之前就开着的会话，跑的还是旧版本。

这时候用新版本才有的参数，会报 `Unknown option`。看起来像参数坏了，其实是会话旧了。
报错里会写明是哪个版本在回答，先看那个数字再判断。

装完之后，一次把这个 workspace 配好：
...
开着 `--show-files` 时，报告会写明跳过了哪些 pattern、压下了多少个改动文件，
并且注明这是主动选定的范围，不是 review 漏掉的。这样缩小过的 review 就不会被读成覆盖缺口。

被截断的文件仍然单独列一份清单。只有那一份代表「证据没拿到」。

如果排除范围大到一个未提交改动都不剩，review 会回落成 branch diff，报告里会写明这件事。
不然一个写太宽的 glob 会让 review 什么都没看，还一声不吭。
